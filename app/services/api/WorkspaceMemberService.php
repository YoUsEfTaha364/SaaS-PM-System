...
class WorkspaceMemberService
{
    ...
    public function addMember(array $validated_data, Workspace $workspace)
    {
        ...
        if (! $user) {
          $this->status=  $this->non_registered($validated_data, $workspace) ?? "empty";
       ...
       $this->status= $this->alreadyMember($user, $workspace) ?? "empty";

        $this->status=$this->registered($validated_data, $workspace, $user) ?? "empty";
    ...
    private function non_registered(array $data, Workspace  $workspace)
    {
        ...
        $url = "front.app/" . $invitation->token;
